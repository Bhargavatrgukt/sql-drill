...
--  INSERT INTO Channels_users_subscribed 
--  (user_id,channel_id)
--  VALUES
--  (1,1),
--  (1,2),
...
-- ("ok chris",1,2),
-- ("Welcome everyone!", 1, 1),   -- Alice in #general
-- ("Sounds good, Chris!", 2, 2), -- Bob in #random
-- ("Let's discuss ideas", 3, 2), -- Chris in #random
-- ("How is everyone doing?", 1, 1), -- Alice in #general
-- ("Do we have any updates?", 2, 1), -- Bob in #general
-- ("Looking forward to it!", 3, 2);  -- Chris in #random
-- querying by select 
...
-- WHERE channel_id 
-- IN (SELECT id FROM Channels where name="#general")
-- ORDER BY post_time DESC;
-- List all channels to which user Alice belongs.
-- SELECT  DISTINCT name  FROM Channels c
-- LEFT JOIN Messages m
-- ON  c.id=m.channel_id
-- WHERE m.user_id 
-- IN (SELECT id FROM Users where name="Alice");
-- List all users that belong to channel #general.
-- WITH CTE AS(
-- 	SELECT  DISTINCT m.user_id  FROM Channels c
-- 	LEFT JOIN Messages m
-- 	ON  c.id=m.channel_id
...
-- SELECT content,channel_id FROM Messages
-- where user_id 
-- IN (SELECT id FROM Users where name="Alice");
-- List all messages in #random by user Bob
-- SELECT content FROM Messages
-- where user_id 
-- IN (SELECT id FROM Users WHERE name="Bob")
-- AND 
-- channel_id IN (SELECT id FROM Channels WHERE name="#random");
